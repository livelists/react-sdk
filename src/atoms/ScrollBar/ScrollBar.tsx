...
import { returnFalse } from '../../utils/helpers/returnFalse';
...
export default class Scrollbars extends Component<IProps, IState> {
    private view: any;

    private trackVertical: any;

    private trackHorizontal: any;

    private thumbVertical: any;

    private thumbHorizontal:any;
    ...
    private hideTracksTimeout:any;

    private detectScrollingInterval:any;
    ...
    componentDidMountUniversal() { // eslint-disable-line react/sort-comp
        const { universal } = this.props as IProps;
        if (!universal) return;
    ...
    addListeners() {
        ...
        if (!getScrollbarWidth({ cacheEnabled: false })) return;
    ...
    handleWindowResize() {
        getScrollbarWidth({ cacheEnabled: false });
        this.forceUpdate();
    }
    ...
    setupDragging() {
        setCSS(document.body, disableSelectStyle);
    ...
    teardownDragging() {
        setCSS(document.body, disableSelectStyleReset);
    ...
    showTracks() {
        clearTimeout(this.hideTracksTimeout);
        setCSS(this.trackHorizontal, { opacity: 1 });
        setCSS(this.trackVertical, { opacity: 1 });
    }
    ...
    hideTracks() {
        ...
        this.hideTracksTimeout = setTimeout(() => {
            setCSS(this.trackHorizontal, { opacity: 0 });
            setCSS(this.trackVertical, { opacity: 0 });
        }, autoHideTimeout);
    }
    ...
    _update(callback:Function) {
        const { onUpdate, hideTracksWhenNotNeeded } = this.props;
        const values = this.getValues();
        if (getScrollbarWidth({ cacheEnabled: false })) {
            ...
            if (hideTracksWhenNotNeeded) {
                ...
                setCSS(this.trackHorizontal, trackHorizontalStyle);
                setCSS(this.trackVertical, trackVerticalStyle);
            }
            setCSS(this.thumbHorizontal, thumbHorizontalStyle);
            setCSS(this.thumbVertical, thumbVerticalStyle);
        }
    ...
    render() {
        const scrollbarWidth = getScrollbarWidth({ cacheEnabled: false });
        ...
        return createElement(tagName, {
            ...props, style: containerStyle, ref: (ref) => {
                this.container = ref;
            }
        }, [
            cloneElement(
            ...
            cloneElement(
                ...
                {
                    key: 'trackHorizontal', ref: (ref:any) => {
